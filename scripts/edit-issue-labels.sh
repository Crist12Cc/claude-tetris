...
# Usage: ./scripts/edit-issue-labels.sh --add-label bug --add-label question --remove-label invalid
#
# The issue number is read from the workflow event payload, not from an
# argument, so this script can never be pointed at a different issue.
#
...
# Fetch valid labels from the repo so Claude can never apply a label it invented
